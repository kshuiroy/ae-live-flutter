...
class DateTimeConverter {
  ...
  static DateTime convertWaitTime(final String dateTimeString) {
    initializeDateFormatting();

    // Attempt to parse as Traditional Chinese format
    // e.g. 2024年4月10日 下午1時15分
    try {
      final DateFormat chineseFormat = DateFormat('yyyy年MM月dd日 ahh時mm分', 'zh');
    ...
    try {
      final DateFormat chineseFormat = DateFormat('yyyy年MM月dd日 ahh时mm分', 'zh');
      return chineseFormat.parse(dateTimeString);
    } catch (_) {}
    ...
    // 10/4/2024 1:15pm
    try {
      final DateFormat englishFormat = DateFormat('dd/MM/yyyy hh:mma');
    ...
    // 10/4/2024 1:15pm
    try {
      final DateFormat generalFormat = DateFormat('yyyy-MM-dd HH:mm');
